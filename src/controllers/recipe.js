...
export const getRecipes = async (req, res) => {
  const { page, perPage, skip, sortBy, sortOrder, filter } = parseQueryOptions(
    req.query,
  );
...
export const getRecipeById = async (req, res, next) => {
  ...
  res.status(200).json({
    ...
    data: { recipes: { ...enrichedRecipe, isFavorite } },
  });
};
...
export const addToFavorites = async (req, res, next) => {
  try {
    ...
    const [enrichedRecipe] = await getEnrichedRecipes([recipeExists]);
    ...
    res.status(200).json({
      ...
      data: {
        recipe: {
          ...(enrichedRecipe.toObject?.() ?? enrichedRecipe),
          isFavorite: false,
        },
        favoritesCount,
      },
...
//   const sortedRecipes = recipes.sort((a, b) => {
//     const aIndex = user.favorites.findIndex(
//       (id) => id.toString() === a._id.toString(),
//     );
//     const bIndex = user.favorites.findIndex(
...
export const removeFavorite = async (req, res, next) => {
  try {
    ...
    const [enrichedRecipe] = await getEnrichedRecipes([recipe]);
